...
def write_fits(filename, data):
    ...
    def add_card(keyword, value, comment=""):
        ...
        card_str = f"{kw_str}= {val_field} / {comment}"
        header_cards.append(f"{card_str:<80}"[:80].encode('ascii'))

    add_card("SIMPLE", True, "File conforms to FITS standard (Geminus build)")
    ...
    header_bytes += b" " * header_padding_len  # 头文件的 padding 必须是 ASCII 空格 (0x20)

    data_bytes = big_endian_data.tobytes()
    data_padding_len = (2880 - (len(data_bytes) % 2880)) % 2880
    data_padding_bytes = b"\x00" * data_padding_len  # 数据的 padding 必须是 Null bytes (0x00)
    ...
    print(f"[Success] FITS 写入完毕: {filename}")
    print(f"Header 大小: {len(header_bytes)} bytes")
    print(f"Data 大小: {len(data_bytes) + len(data_padding_bytes)} bytes")
